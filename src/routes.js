...
/**
 * GET /api/devices
 * List all registered devices with reading counts and last-seen time.
 */
router.get('/devices', async (req, res) => {
    try {
...
/**
 * GET /api/sensors/latest
 * Get the most recent reading for every device.
 */
router.get('/sensors/latest', async (req, res) => {
    try {
        const rows = await getLatestPerDevice();
 ...
 * Query params: device_id, limit (default 100), offset (default 0)
 */
router.get('/sensors', async (req, res) => {
    try {
        const deviceId = req.query.device_id || null;
